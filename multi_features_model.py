...
def build_input_from_segments(history, emotions, actions, reply, candidate_emotion, candidate_act, tokenizer, lm_labels=False, with_eos=True):
        ...
        for dialog in dataset:
            ...
            for utterance in dialog["utterances"]:
                ...
                for j, candidate in enumerate(utterance["candidates"][-num_candidates:]):
                    ...
                    instance, _ = build_input_from_segments(history, emotions, actions, candidate, candidate_emotion, 
                    ...
                    if len(instance["input_ids"]) > gpu_max_length:
                        ...
                        instance, _ = build_input_from_segments(truncated_history, emotions, actions, truncated_candidate, candidate_emotion, 
                                                                candidate_act, tokenizer, lm_labels)
...
def train():
    ...
    if config.local_rank in [-1, 0] and config.n_epochs > 0:
        os.rename(config.log_dir + '/' + checkpoint_handler._saved[-1][-1], 'multi_logger/all_model_128.bin')  # TODO: PR in ignite to have better access to saved file paths (cleaner)
        tb_logger.close()
